...
import src.modules.stgcn.tools
import src.modules.stgcn.tools.utils as utils

import cv2

# sys.path.append()
#
# os.environ['PATH'] = os.environ['PATH'] + ';' + self.arg.openpose + '/build/x64/Release;' + self.arg.openpose + '/build/bin;'

class StGcn(IO):

    def action_recognize(self, data_numpy):
        ...
        # pose_tracker = naive_pose_tracker(data_frame=pose_array.shape[0])
        # for i, pose in enumerate(pose_array):
        #     # normalization
        #     pose[:, :, 0] = pose[:, :, 0] / video.shape[2]
        #     pose[:, :, 1] = pose[:, :, 1] / video.shape[1]
        #     pose[:, :, 0:2] = pose[:, :, 0:2] - 0.5
        #     pose[:, :, 0][pose[:, :, 2] == 0] = 0
        #     pose[:, :, 1][pose[:, :, 2] == 0] = 0
        #     pose_tracker.update(pose, i)
...
if __name__ == '__main__':
    test = StGcn()
    test.get_parser()
    a = np.load('stgcn_data_numpy_original.npy')
    test.action_recognize(a)
