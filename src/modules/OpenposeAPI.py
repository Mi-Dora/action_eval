...
class OpenPoseEstimator(object):
    def __init__(self, op_path, DEBUG=False):
        ...
        params = dict(model_folder='../../weights', model_pose='COCO')
    ...
    def estimate_video(self, video_file, short_edge=512, only_prior_person=True):
        ...
        while True:
            ...
            if self.DEBUG:
                ...
                cv2.waitKey(1)
            return pose_list, norm_pose_list

    def estimate_image(self, img, only_prior_person=True):
        # pose estimation
        datum = self.op.Datum()
        ...
        if only_prior_person:
            pose = self.__get_prior_person(multi_pose)
            return pose, self.__pose_normalization(pose)
        return multi_pose, self.__pose_normalization(multi_pose)
    ...
    def __pose_normalization(self, pose):
        ...
        pose[:, :, 0] = pose[:, :, 0] / self.W
        pose[:, :, 1] = pose[:, :, 1] / self.H
...
if __name__ == '__main__':
    openpose = OpenPoseEstimator('D:\\Desktop\\openpose')
    openpose.estimate_video("D:\\Desktop\\video_20210422\\7.mp4")
